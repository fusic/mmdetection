...
class BaseEncoder(nn.Module):
    ...
    def infe_speed(self, input_shape=(1, 3, 256, 256), use_cuda: Optional[bool]=True, warmup_num=20, repeat_num=100):
        ...
        model_infe.append(copy.deepcopy(self.conv_1).eval())
        module_list.append('conv1')
        model_infe.append(copy.deepcopy(self.layer_1).eval())
        module_list.append('layer_1')
        model_infe.append(copy.deepcopy(self.layer_2).eval())
        module_list.append('layer_2')
        model_infe.append(copy.deepcopy(self.layer_3).eval())
        module_list.append('layer_3')
        model_infe.append(copy.deepcopy(self.layer_4).eval())
        module_list.append('layer_4')
        model_infe.append(copy.deepcopy(self.layer_5).eval())
        module_list.append('layer_5')
        model_infe.append(copy.deepcopy(self.conv_1x1_exp).eval())
        module_list.append('fc_layer')

        if use_cuda:
            input = input.to('cuda:0')

            for item in model_infe:
                item = item.to('cuda:0')
        ...
        if use_cuda:
            with torch.no_grad():
                ...
                for module, layer_name in zip(model_infe, module_list):
                    ...
                    print('{}:  input {} -> output {} || time cost:{} seconds'.format(layer_name, input.shape[1:],
                                                                                      output.shape[1:],
                                                                                      round(t_cost / repeat_num * 1000,
                                                                                            3)))
                    input = output
                    warnings.warn('-------------------------------------------------')
                ...
                warnings.warn('='*65)
                print('{}: time cost:{} milliseconds'.format(self.__class__.__name__,
                                                             round(t_cost / repeat_num * 1000, 3)))
                warnings.warn('='*65)
        else:
            with torch.no_grad():
