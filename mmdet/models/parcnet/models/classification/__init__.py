...
# from mmdet.models.parcnet.utils.download_utils import get_local_path
# from mmdet.models.parcnet.utils import logger
# from mmdet.models.parcnet.utils.common_utils import check_frozen_norm_layer
# from mmdet.models.parcnet.utils.ddp_utils import is_master
...
def build_classification_model(opts, *args, **kwargs):
    model_name = getattr(opts, "model_classification_name", "edgeformer")
    model = None
    is_master_node = False
    print("="*65, CLS_MODEL_REGISTRY, model_name)
    if model_name in CLS_MODEL_REGISTRY:
        cls_act_fn = getattr(opts, "model_classification_activation_name", "swish")
        if cls_act_fn is not None:
            # Override the general activation arguments
            gen_act_fn = getattr(opts, "model_activation_name", "relu")
            gen_act_inplace = getattr(opts, "model_activation_inplace", False)
            gen_act_neg_slope = getattr(opts, "model_activation_neg_slope", 0.1)

            setattr(opts, "model_activation_name", cls_act_fn)
            setattr(opts, "model_activation_inplace", getattr(opts, "model_classification_activation_inplace", False))
            setattr(opts, "model_activation_neg_slope", getattr(opts, "model_classification_activation_neg_slope", 0.1))

            model = CLS_MODEL_REGISTRY[model_name](opts, *args, **kwargs)

            # Reset activation args
            setattr(opts, "model_activation_name", gen_act_fn)
            setattr(opts, "model_activation_inplace", gen_act_inplace)
            setattr(opts, "model_activation_neg_slope", gen_act_neg_slope)
        else:
            model = CLS_MODEL_REGISTRY[model_name](opts, *args, **kwargs)
    else:
...
for file in os.listdir(models_dir):
    ...
    if (
        ...
        module = importlib.import_module("mmdet.models.parcnet.models.classification." + model_name)
