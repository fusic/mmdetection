...
def get_configuration(opts) -> Dict:
    mode = opts.get("model.classification.edge.mode", "outer_frame_v1")
    if mode is None:
        warnings.warn("Please specify mode")

    mode = mode.lower()
    if mode in ['outer_frame_v1', 'outer_frame_v2']:
        scale = opts.get("model_classification_edge_scale", 'scale_s')
        kernel = opts.get("model_classification_edge_kernel", "gcc_ca")
        fusion = opts.get("model_classification_edge_fusion", "concat")
        instance_kernel = opts.get("model_classification_edge_instance_kernel", "interpolation_bilinear")
        mid_mix = opts.get("model_classification_edge_mid_mix", False)
        use_pe = opts.get("model_classification_edge_use_pe", True)
