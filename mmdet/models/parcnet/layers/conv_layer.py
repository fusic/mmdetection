...
class ConvLayer(BaseLayer):
    def __init__(self,
        ...
        if groups and in_channels % groups != 0:
            warnings.warn('Input channels are not divisible by groups. {}%{} != 0 '.format(in_channels, groups))
        if groups and out_channels % groups != 0:
            warnings.warn('Output channels are not divisible by groups. {}%{} != 0 '.format(out_channels, groups))
        ...
        self.norm_name = None
        if use_norm:
            norm_layer = get_normalization_layer(num_features=out_channels, opts_model_normalization_name=opts_model_normalization_name,
        ...
        self.act_name = None
        act_type = opts_model_activation_name

        if act_type is not None and use_act:
            neg_slope = opts_model_activation_neg_slope
            inplace = opts_model_activation_inplace
